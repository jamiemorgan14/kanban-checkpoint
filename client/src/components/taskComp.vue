    ...
    <div class="col">
      <ul>
        <li class="task inline">
          <i v-if="!editedTask.completed" @click="editedTask.completed = !editedTask.completed, markComplete(editedTask)"
          ...
          <div class="inline">
            <div class="inline" v-if="!editTaskDetails" @dblclick="editTaskDetails = !editTaskDetails, !showTaskDetails"
              @mouseover="showTaskDetails = true" @mouseleave="showTaskDetails = false" v-bind:class="{completed: editedTask.completed}">
              <h5>{{taskData.description}}</h5>
            </div>
            <div class="inline" v-if="editTaskDetails">
              <form class="mt-2" @submit.prevent="editTask(editedTask)">
                <input autofocus="autofocus" type="text" v-model="editedTask.description" required>
                <div class="inline">
        ...
        <form @submit.prevent="addComment" v-if="addSubCommentForm">
          <input autofocus="autofocus" required v-model="newComment.description" type="text">
          <button @click="addSubCommentForm = false" type="submit" class="btn-sm btn-outline-dark mx-1">
